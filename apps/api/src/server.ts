...
app.get('/api/routes', async (request) => { requireRole(request, ['owner', 'admin', 'operator', 'auditor']); return db.prepare(`SELECT r.id, r.public_model, r.enabled, r.priority, m.provider, m.upstream_id, m.capabilities_json FROM routes r JOIN models m ON m.id = r.model_id ORDER BY r.enabled DESC, r.priority DESC, r.public_model ASC`).all(); });
app.put('/api/routes/reorder', async (request) => {
  requireRole(request, ['owner', 'admin', 'operator']);
  const body = z.object({
    routes: z.array(z.object({ id: z.string(), priority: z.number().int().min(0).max(10000) })).min(1)
  }).parse(request.body);
  db.exec('BEGIN IMMEDIATE');
  try {
    const updateStmt = db.prepare('UPDATE routes SET priority = ? WHERE id = ?');
