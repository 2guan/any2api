...
export async function registerMessagesRoutes(app: FastifyInstance) {
  app.post('/v1/messages', async (request, reply) => {
    ...
    const usage = estimateTokens(openaiMessages, textContent);
...
async function streamAnthropic(gatewayReq: { model: string; messages: Array<{ role: string; content: unknown }>; stream: boolean }, reply: FastifyReply, options: { kind: 'api'; apiKeyId?: string }) {
  ...
  const msgId = `msg_${Math.random().toString(36).substring(2, 11)}`;
  const initialUsage = estimateTokens(gatewayReq.messages, '');

  try {
    reply.raw.write(`event: message_start\ndata: ${JSON.stringify({ type: 'message_start', message: { id: msgId, type: 'message', role: 'assistant', content: [], model: gatewayReq.model, stop_reason: null, usage: { input_tokens: initialUsage.prompt_tokens, output_tokens: 0 } } })}\n\n`);
    ...
    const usage = estimateTokens(gatewayReq.messages, textContent);

    reply.raw.write(`event: content_block_stop\ndata: ${JSON.stringify({ type: 'content_block_stop', index: 0 })}\n\n`);
    reply.raw.write(`event: message_delta\ndata: ${JSON.stringify({ type: 'message_delta', delta: { stop_reason: 'end_turn', stop_sequence: null }, usage: { output_tokens: usage.completion_tokens } })}\n\n`);
